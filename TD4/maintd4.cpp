...
private:
    ...
    void buildBHV(Node* node, int start, int end) {
        ...
        node->left = new Node();
        node->right = new Node();
        buildBHV(node->left, start, p);
        buildBHV(node->right, p, end);
    }
...
public:
    ...
    Vector computeBC(const TriangleIndices& t) {
        return (vertices[t.vtxi] + vertices[t.vtxj] + vertices[t.vtxk]) / 3.0;
    ...
    Intersection intersect(const Ray& r, bool& is_in) const override {
        ...
        while (!bvh.empty()) {
            ...
            if (!node->left && !node->right) {
                ...
                if (hl && tl < closest_t) bvh.push_back(node->left);
                if (hr && tr < closest_t) bvh.push_back(node->right);
            }
        }
        if (I.intersects) {
            is_in = dot(r.u, I.N) > 0;
        ...
        Vector getColour(const Ray& ray, int depth, Vector lpos, double I, bool& is_in) {
            ...
            if (In.intersects) {
                ...
                if (isShadow(In.P, In.N, lpos, is_in)) {
                    ...
                    colour = colour + (In.rho * getColour(random_ray, depth - 1, lpos, I, is_in));
                    return colour;        
                }
            }
